...
import com.example.book_store1.CartItem;
import com.example.book_store1.CartManager;
...
public class MentalBeHappyActivity extends MentalBooksDetailsActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // ✅ Cart button logic inside included layout
        View cardInclude = findViewById(R.id.cardInclude); // this is the <include> id
        ImageButton cartButton = cardInclude.findViewById(R.id.imageButtonMentalBeHappy); // this is the 🛒 button in the card layout

        cartButton.setOnClickListener(v -> {
            CartItem item = new CartItem(
                    "Be Happy",
                    "Imagine possessing the priceless antidote. Imagine seeing a part of paradise, a part that only private people reach...",
                    5.0,
                    "be_happy"
            );

            CartManager.getInstance().addItem(item);
            Toast.makeText(MentalBeHappyActivity.this, "Added to Cart", Toast.LENGTH_SHORT).show(); // no activity started
        });
    }
